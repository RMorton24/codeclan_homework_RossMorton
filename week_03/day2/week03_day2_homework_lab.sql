...
SELECT
	t.name AS team_name,
	count(e.id) AS number_employees_in_team
FROM employees AS e LEFT JOIN teams AS t
ON e.team_id  = t.id 
GROUP BY t.name;
...
Hint
If you GROUP BY teams.id, because it’s the primary key, you can SELECT any other column of teams that 
you want (this is an exception to the rule that normally you can only SELECT a column that you GROUP BY).*/
SELECT
	t.id AS team_id ,
	t.name,
	count(e.id)*cast(t.charge_cost AS decimal) AS total_day_charge
FROM employees AS e LEFT JOIN teams AS t
ON e.team_id  = t.id 
GROUP BY t.id;
...
ON e.team_id  = t.id 
GROUP BY t.id
HAVING count(e.id)*cast(t.charge_cost AS decimal) > 5000; 

/* Old convoluted way....
SELECT 
	ti.name,
	ti.id,
	ex.number_employees_in_team,
	ex.number_employees_in_team * cast(ti.charge_cost AS decimal) AS total_day_charge
FROM (SELECT
			t.id AS team_id ,
			count(e.id) AS number_employees_in_team
		FROM employees AS e LEFT JOIN teams AS t
		ON e.team_id  = t.id 
		GROUP BY t.id) AS ex LEFT JOIN teams  AS ti
ON ex.team_id = ti.id
WHERE ex.number_employees_in_team * cast(ti.charge_cost AS decimal) > 5000;*/
...
SELECT 
	count(DISTINCT employee_id) AS number_employees_committee
